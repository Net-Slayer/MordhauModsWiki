...
export function getTreeData(collection, entries) {
  ...
  const flatData = [
    {
      ...
      isDir: true,
      isRoot: true,
      children: [], // Initialize children array for the root folder
    },
    ...Object.entries(dirs).map(([key, value]) => ({
      ...
      isDir: true,
      isRoot: false,
      children: [], // Initialize children array for each folder
    })),
  ...
  const parentsToChildren = flatData.reduce((acc, node) => {
    ...
    if (acc[parent]) {
      acc[parent].children.push(node); // Push node to the children array of the parent
    } else {
      acc[parent] = { children: [node] }; // Initialize children array for the parent
    }
    return acc;
  }, {});
  ...
  function reducer(acc, value) {
    const node = value;
    let children = [];
    if (parentsToChildren[node.path] && parentsToChildren[node.path].children) {
      children = parentsToChildren[node.path].children.reduce(reducer, []);
    }
  ...
  const treeData = parentsToChildren[''].children.reduce(reducer, []);
